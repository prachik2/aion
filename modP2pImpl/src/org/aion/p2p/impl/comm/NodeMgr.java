...
public class NodeMgr implements INodeMgr {
                ...
                try {
                    sb.append(
                            String.format(
                                    ...
                                    n.getBestBlockHash() == null
                                            ? ""
                                            : bytesToHex(n.getBestBlockHash()),
                                    n.getIpStr(),
                                    n.getPort(),
                                    n.getConnection(),
                                    n.getBinaryVersion(),
    ...
    public synchronized void addTempNode(final Node _n) {
        if (tempNodes.size() < maxTempNodes) tempNodes.add(_n);
    }
    ...
    public Node allocNode(String ip, int p0) {
        ...
        if (seedIps.contains(ip)) n.setFromBootList(true);
        return n;
    }
    ...
    public synchronized void moveInboundToActive(int _channelHashCode, final IP2pMgr _p2pMgr) {
        ...
        if (node != null) {
            ...
            if (previous != null)
                ...
                _p2pMgr.closeSocket(
                        node.getChannel(), "inbound -> active, ip " + node.getIpStr() + " exits");
            else {
                if (_p2pMgr.isShowLog())
                    System.out.println(
                            "<p2p inbound -> active node-id="
                                    + node.getIdShort()
                                    + " ip="
                                    + node.getIpStr()
                                    + ">");
    ...
    public synchronized void moveOutboundToActive(
            int _nodeIdHash, String _shortId, final IP2pMgr _p2pMgr) {
